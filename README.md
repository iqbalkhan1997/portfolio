...
- [ ] `index.html` → replace `YOUR-LINKEDIN-HANDLE` in the Contact section with your real LinkedIn URL.
- [ ] `index.html` → replace `YOUR-GITHUB-USERNAME` in the Contact section with your real GitHub username.
...
## Publishing checklist (once you have your GitHub login)

This folder is already a local git repo with an initial commit — publishing is just:

1. **Create the GitHub repo** (on github.com, not locally):
   - For a personal top-level site at `https://<username>.github.io`: create a repo named **exactly** `<username>.github.io` (e.g. `ghourikhan.github.io`).
   - For a project-style URL instead (`https://<username>.github.io/portfolio`): name the repo anything, e.g. `portfolio`.
   - Leave it empty — don't add a README/license from GitHub's UI, since this folder already has one.

2. **Connect and push** (run from inside this folder):
   ```bash
   git remote add origin https://github.com/<username>/<repo-name>.git
   git branch -M main
   git push -u origin main
   ```
   (First push will prompt for GitHub login — a personal access token, not your account password, since GitHub retired password auth for git operations. GitHub will show you how to generate one, or use `gh auth login` if the GitHub CLI is installed.)
...
4. **Wait ~1 minute**, then visit:
   - `https://<username>.github.io` (if you used the special repo name), or
   - `https://<username>.github.io/<repo-name>` (project-style).
...
1. Add a file named `CNAME` (no extension) to this folder containing just your domain, e.g. `ghourikhan.dev`.
2. At your domain registrar, add a `CNAME` record pointing your domain (or `www`) at `<username>.github.io`.
